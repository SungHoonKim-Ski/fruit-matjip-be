...
public class DeliveryAppService {
    ...
    public DeliveryReadyResponse ready(String uid, DeliveryReadyRequest request) {
        ...
        validateDeliveryHour(request.deliveryHour());
    ...
    private void validateDeliveryHour(Integer deliveryHour) {
        int startHour = deliveryConfigDto.getStartHour();
        int endHour = deliveryConfigDto.getEndHour();
        if (deliveryHour == null || deliveryHour < startHour || deliveryHour > endHour) {
            throw new UserValidateException("배달 수령 시간은 " + formatTime(startHour, deliveryConfigDto.getStartMinute())
                + "~" + formatTime(endHour, deliveryConfigDto.getEndMinute()) + " 사이여야 합니다.");
        }
    }
