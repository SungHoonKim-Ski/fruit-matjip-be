...
class ProductsAppServiceTest {
    ...
    class GetProducts {
        ...
        void getProducts_ReturnsVisibleProducts() {
            ...
            // when
            ProductListResponse result = productsAppService.getProducts(from, to);
    ...
    @Nested
    @DisplayName("getProductKeywords - 상품 키워드 조회")
    class GetProductKeywords {

        @Test
        @DisplayName("상품 키워드 목록 조회 성공")
        void getProductKeywords_Success() {
            // given
            ProductKeyword keyword1 = ProductKeyword.builder().name("과일").build();
            ProductKeyword keyword2 = ProductKeyword.builder().name("채소").build();

            given(productKeywordService.findAll()).willReturn(List.of(keyword1, keyword2));

            // when
            ProductKeywordResponse result = productsAppService.getProductKeywords();
        ...
        @Test
        @DisplayName("키워드가 없는 경우 빈 목록 반환")
        void getProductKeywords_Empty() {
            // given
            given(productKeywordService.findAll()).willReturn(List.of());
